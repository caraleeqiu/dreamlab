...
const PACKAGES = [
  { id: 'starter',   credits: 100,  price: 9.9,   label: '入门包',   popular: false, bonus: 0 },
  { id: 'standard',  credits: 300,  price: 25,    label: '标准包',   popular: true,  bonus: 30 },
  { id: 'pro',       credits: 800,  price: 59,    label: '专业包',   popular: false, bonus: 100 },
  { id: 'team',      credits: 2000, price: 128,   label: '团队包',   popular: false, bonus: 300 },
]
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">购买积分用于生成视频</p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-br from-violet-900/40 to-zinc-900 border border-violet-800/50 mb-8">
        <div className="flex items-end gap-3">
          <div>
            <p className="text-sm text-zinc-400 mb-1">当前余额</p>
            <div className="text-5xl font-bold text-white">{credits}</div>
            <p className="text-sm text-zinc-500 mt-1">积分</p>
          </div>
      ...
      {/* Packages */}
      <h2 className="text-sm font-medium text-zinc-400 mb-4">选择套餐</h2>
      <div className="grid grid-cols-2 gap-3 mb-8">
        {PACKAGES.map(pkg => (
          ...
          >
            ...
            <div className="mb-3">
              <div className="text-sm text-zinc-400">{pkg.label}</div>
              <div className="text-2xl font-bold text-white mt-0.5">
                {pkg.credits}
                {pkg.bonus > 0 && <span className="text-sm font-normal text-green-400 ml-1">+{pkg.bonus}赠送</span>}
              </div>
      ...
      {isDev && (
        <div className="p-3 rounded-lg border border-dashed border-zinc-700 mb-4">
          <div className="flex items-center justify-between">
            <span className="text-xs text-zinc-500">开发模式 · 快速充值</span>
            <Button
              onClick={handleDevTopUp}
              disabled={loading === 'dev'}
              className="h-7 text-xs bg-zinc-700 hover:bg-zinc-600 text-zinc-200"
            >
              {loading === 'dev' ? <Loader2 size={11} className="animate-spin" /> : '+200 积分（测试）'}
            </Button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 p-3 rounded-lg bg-green-900/20 border border-green-800/50 mb-8">
        ...
        <p className="text-xs text-green-400">新用户注册即赠 20 积分 · 首个网红免费创建</p>
      </div>

      {/* Transaction history */}
      <h2 className="text-sm font-medium text-zinc-400 mb-4">消费记录</h2>
